...
rms = np.sqrt(np.sum((ref - net.x) ** 2))
check(rms, 1e-4)
...
ref = np.interp(net.t, ref[:,0],ref[:,1])
rms = np.sqrt(np.sum((ref[1:] - val[1:]) ** 2))
check(rms, 1e-1)
...
rms1 = np.sqrt(np.sum((ref1[1:] - val1[1:]) ** 2))
rms2 = np.sqrt(np.sum((ref2[1:] - val2[1:]) ** 2))
rms = np.sqrt(rms_r ** 2 + rms_i ** 2)
check(rms, 1e-4)
